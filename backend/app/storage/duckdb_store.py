...
class DuckDBStore:
    ...
    def create_run(self, run: IngestionRun) -> None:
        ...
        self.con.execute("INSERT OR REPLACE INTO ingestion_runs VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", vals)
        self.con.commit()

    update_run = create_run

    def get_run(self, ingestion_id: str) -> dict[str, Any] | None:
        row = self.con.execute("SELECT * FROM ingestion_runs WHERE ingestion_id = ?", [ingestion_id]).fetchone()
        return dict(row) if row else None

    def insert_points(self, points: list[SignalPoint]) -> None:
        if points:
            self.con.executemany("INSERT INTO signal_points VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [(p.press_id, p.source_type, p.source_file, p.system, p.component, p.device, p.signal_id, p.signal_name, _dt(p.timestamp_utc_or_local), p.numeric_value, p.unit, p.machine_state, p.system_state) for p in points])
            self.con.commit()

    def insert_state_updates(self, updates: list[StateUpdate]) -> None:
        if updates:
            self.con.executemany("INSERT INTO state_updates VALUES (?, ?, ?, ?, ?, ?)", [(u.press_id, u.scope, u.system, u.state, _dt(u.timestamp), u.source) for u in updates])
            self.con.commit()

    def insert_diagnostics(self, diagnostics: list[Diagnostic]) -> None:
        if diagnostics:
            self.con.executemany("INSERT INTO ingestion_diagnostics VALUES (?, ?, ?, ?, ?, ?, ?)", [(d.ingestion_id, d.level, d.code, d.message, d.source_file, d.row_number, json.dumps(d.details)) for d in diagnostics])
            self.con.commit()

    def rebuild_catalog(self) -> None:
        self.con.execute("DELETE FROM signal_catalog")
        self.con.execute("""
    ...
    def rebuild_state_intervals(self) -> None:
        self.con.execute("DELETE FROM state_intervals")
        rows = self.con.execute("SELECT press_id, scope, system, state, timestamp, source FROM state_updates ORDER BY press_id, scope, coalesce(system, ''), timestamp").fetchall()
        ...
        if active:
            intervals.append((active[0], active[1], active[2], active[3], _dt(active[4]), None, None, active[5]))
        if intervals:
            self.con.executemany("INSERT INTO state_intervals VALUES (?, ?, ?, ?, ?, ?, ?, ?)", intervals)
        self.con.commit()
    ...
    def list_systems(self) -> list[dict[str, Any]]:
        return [dict(r) for r in self.con.execute("SELECT system, count(DISTINCT signal_id) AS signal_count, sum(sample_count) AS sample_count FROM signal_catalog GROUP BY system ORDER BY system").fetchall()]

    def list_components(self, system: str) -> list[dict[str, Any]]:
        return [dict(r) for r in self.con.execute("SELECT component, count(DISTINCT signal_id) AS signal_count, sum(sample_count) AS sample_count FROM signal_catalog WHERE system = ? GROUP BY component ORDER BY component", [system]).fetchall()]
    ...
    def list_signals(self, system: str | None = None, component: str | None = None, source_type: str | None = None, search: str | None = None, start: datetime | None = None, end: datetime | None = None) -> list[dict[str, Any]]:
        ...
        return [dict(r) for r in self.con.execute(f"SELECT * FROM signal_catalog{where} ORDER BY system, component, signal_name LIMIT 1000", params).fetchall()]

    def get_signal(self, signal_id: str) -> dict[str, Any] | None:
        row = self.con.execute("SELECT * FROM signal_catalog WHERE signal_id = ?", [signal_id]).fetchone()
        return dict(row) if row else None
    ...
    def series(self, signal_id: str, max_points: int, start: datetime | None = None, end: datetime | None = None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        row = self.con.execute(f"SELECT count(*), min(numeric_value), max(numeric_value), avg(numeric_value), min(timestamp_utc_or_local), max(timestamp_utc_or_local) FROM signal_points WHERE {where}", params).fetchone()
        ...
        latest = self.con.execute(f"SELECT numeric_value FROM signal_points WHERE {where} ORDER BY timestamp_utc_or_local DESC LIMIT 1", params).fetchone()[0]
        all_rows = self.con.execute(f"SELECT timestamp_utc_or_local AS timestamp, numeric_value AS value, machine_state, system_state FROM signal_points WHERE {where} ORDER BY timestamp_utc_or_local", params).fetchall()
    ...
    def intervals(self, system: str | None = None, start: datetime | None = None, end: datetime | None = None) -> list[StateInterval]:
        ...
        where = " WHERE " + " AND ".join(clauses) if clauses else ""
        rows = self.con.execute(f"SELECT * FROM state_intervals{where} ORDER BY start_timestamp", params).fetchall()
    ...
    def diagnostics(self, ingestion_id: str | None = None, level: str | None = None, source_file: str | None = None, code: str | None = None) -> list[dict[str, Any]]:
        ...
        where = " WHERE " + " AND ".join(clauses) if clauses else ""
        return [dict(r) for r in self.con.execute(f"SELECT * FROM ingestion_diagnostics{where} ORDER BY row_number LIMIT 1000", params).fetchall()]
