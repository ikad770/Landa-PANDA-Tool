...
export function renderAnalysisShell(root) {
  root.innerHTML = `<div class="analysis-shell">
    ...
    <section id="uploadWorkspace" class="workspace-grid">
      <article class="upload-console panel-glass"><p class="panel-kicker">INPUT REQUIREMENTS</p><h2>Prepare Service Radar analysis</h2><p class="panel-subtitle">Select one autocollect ZIP and one Rules Excel workbook. Analyze remains locked until both files are ready.</p><div class="upload-stack">
...
export function getVisualStage(progress = {}) {
  const stage = progress.stage || 'rules_loading';
...
export function updateProgressPresentation(progress = {}) {
  ...
  document.getElementById('processingHeadline').textContent = visual.label;
